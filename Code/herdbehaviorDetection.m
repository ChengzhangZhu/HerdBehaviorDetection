function herdbehaviorDetection(data, label, startDate ,timeLine, t)
...
endDate = startDate + t;
% select data
selectData =  data(timeLine>=startDate&timeLine<=endDate,:);
emptyIndex = find(sum(selectData,1)==0);
selectData(:,emptyIndex) = [];
emptyIndex = find(selectData(1,:)==0);
selectData(:,emptyIndex) = [];
selectData = (selectData(2:end,:) - selectData(1:end-1,:))./selectData(1:end-1,:);
emptyIndex = find(sum(selectData,1)==0);
selectData(:,emptyIndex) = [];
emptyIndex = find(sum(selectData,2)==0);
selectData(emptyIndex,:) = [];
selectLabel = label;
